...
const summarize = async (req: Request, res: Response) => {
  try {
    ...
    if (!text && !file) {
      res
        .status(400)
        .json({ error: "Please provide either text or a file to summarize." });
      return;
    }
    ...
    promptText +=
      "Please synthesize and summarize the provided content based on the following custom requirements:";
    promptText += `\n1. **Format/Structure**: ${
...
Make extensive use of bold words to anchor readability.`;
    ...
    res.status(statusCode).json({
      status: statusCode,
      error:
        error.message || "An unexpected error occurred during summarization.",
    });
  }
};
